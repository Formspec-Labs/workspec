...
fn benefits_business_calendar_round_trips() {
    ...
    assert_eq!(
        mlk.rule.as_deref(),
        Some("nthWeekday(3, monday, january)")
    );

    // Observed holiday
    let july3 = doc.holidays.iter().find(|h| h.observed).expect("observed holiday");
    assert_eq!(july3.date.as_deref(), Some("2026-07-03"));
    assert!(july3.observed);

    // Operating hours
    let hours = doc.operating_hours.as_ref().expect("operating hours present");
...
fn business_calendar_serialization_round_trip() {
    ...
    assert_eq!(doc.wos_business_calendar, deserialized.wos_business_calendar);
