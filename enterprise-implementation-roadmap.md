...
| **WOS Runtime Companion** | ~2,200 | **1** | **1.3 Workflow Process** | 📋 Spec + 🟦 partial engine (WorkflowProcess model, host traits) |
| **WOS sidecars (9 total)** | ~3,500 | **1, 4** | **1.3, 4.5** | 📋 Spec + schema + fixtures (Business Calendar, Notification Template, Policy Params, etc.) |
| **Formspec Coprocessor** | 0 | **1** | **1.3 Handoff** | 📋 Specified in Runtime Companion S15 — implementation prerequisite for Phase 1.3 |
...
- 📋 **[WOS]** Case object created from each completed submission — WOS Kernel S4-S5 defines lifecycle topology and case state model; WorkflowProcess serialization in Runtime S3. **Requires:** implementing the Runtime S15 Formspec Coprocessor handoff.
...
| WOS Runtime Companion | Governs Phase 1.3 | 📋 **[Added 2026-04-10]** WorkflowProcess serialization, event delivery contract, action execution model, host interfaces (InstanceStore, ContractValidator, EventQueue, etc.). |
| Formspec Coprocessor | **Enables** Phase 1.3 | 📋 **[Added 2026-04-10, updated 2026-04-12]** Runtime Companion S15 specifies the handoff between Formspec submissions and WOS workflow processes. Phase 1.3 now depends on implementing this protocol rather than writing it. |

No phase in this proposal contradicts an accepted ADR. Phase 1 case management (1.3) ~~introduces a new domain object (case) that is not yet defined in ADR-0006 (Core Data Model). A new ADR or addendum to ADR-0006 should be drafted before Phase 1.3 implementation begins, defining case object ownership boundaries.~~ **[Updated 2026-04-10, revised 2026-04-12]** should implement WOS Kernel and Governance specs as the case management foundation. The WOS WorkflowProcess (Runtime S3) defines the case object; WOS Governance S10 defines the task lifecycle; Runtime Companion S15 defines the Formspec Coprocessor handoff. The remaining requirement for Phase 1.3 is implementation, not additional case-object protocol design.
