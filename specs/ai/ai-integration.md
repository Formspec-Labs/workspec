...
4. Every precondition evaluation MUST produce a provenance record identifying the capability, the expression, and the resolved value. When a precondition blocks an invocation, the record's `outcome` MUST be `preconditionNotSatisfied` so that audit tooling can distinguish a declarative gate from an agent failure.
