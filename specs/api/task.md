...
- `deadline`: legacy single-value SLA deadline (governance S10.3). DEPRECATED for multi-SLA workflows — use `deadlines[]` for the full set; `deadline` is retained for backward compatibility and typically equals `deadlines[0].at` (the earliest enforcing deadline).
