...
- **`AutonomyEvent`** — governance aggregate view of an agent autonomy-level transition (escalated, demoted, restored). Companion to the per-event Facts-tier provenance records (`autonomyEscalation`, `autonomyDemotion`); this is the governance-query shape.
...
`PolicyParameterSummary` additionally carries optional `type` (`"numeric" | "string" | "boolean" | "duration"`) and `unit` (string, e.g. `"USD"`, `"days"`, `"percent"`) fields for numeric-parameter disambiguation and cross-case comparison.
...
Carries `id` (URN), `agentId` (URN of the agent declaration whose autonomy changed), `eventKind` (closed `escalated | demoted | restored`), `occurredAt` (RFC 3339 UTC), `previousLevel` / `newLevel` (closed `AutonomyLevel`), `trigger` (closed `drift | confidence-breach | circuit-breaker | manual | calibration-expired`), and optional `rationale` (prose; present when `trigger == manual`). Read-only at the public API layer — the runtime emits these; per-event detail lives in the Facts-tier `autonomyEscalation` and `autonomyDemotion` provenance records. Companion shape to `AgentView` — the aggregate query surface for "show me every autonomy change for agent X since calibration".
...
| `CircuitBreakerSpec` | `errorRateThreshold` (number 0-1), `evaluationWindowSeconds` (integer ≥ 1), `minimumInvocations` (integer ≥ 1), `probeCount` (integer ≥ 1) — rate-over-window model per advanced-governance.md §11.2. `currentStatus` uses `closed \| open \| half-open`. | none — closed |
...
- `DeonticConstraintSummary` projects from `wos-workflow.schema.json#/$defs/DeonticConstraint` — `id`, `kind`, `actor`, `expression`, `condition`, `defeasible`, `citationRefs` carry through; the kernel-only `x-legalruleml-iri` mapping is not projected. Not byte-pinned at the object level; the closed `DeonticKind` enum it nests IS byte-pinned through `DeonticKind`. `DeonticConstraintSummary` additionally carries an optional `onViolation` field with closed-with-vendor-extension enum `reject | escalate-to-human | switch-to-assistive | flag` — the prescribed response when a deontic constraint is breached at runtime, projected from the workflow's `agents[].deonticConstraints[].onViolation` declaration; vendor extensions use the `^x-[a-z]+-` pattern.
