...
Appeals are filed against a specific workflow process (must be in a completed lifecycle state — `WOS-1409` on non-completed processes). The appeal record links back to the process via `processId` (process URN) and to the appellant via `appellantRef` (ActorRef URN per ADR 0082 D-9). When routed through a governed review process, the adjudicator is recorded via `adjudicatorRef`.
...
`GET /api/v1/cases/{case_id}/processes/{process_id}/appeals` is cursor-paginated per ADR 0082 D-7. Filters: `status` (most common: `pending` for unadjudicated appeals). Cursors are deploy-lifetime stable; `WOS-1410` on expiry triggers client restart from the top.

## Pagination

`GET /api/v1/cases/{case_id}/processes/{process_id}/appeals` uses cursor pagination per `api/pagination.schema.json`. Cursors are opaque, single-use within the issuing deploy. Default ordering is `createdAt` ascending so paginating clients observe a stable monotone walk. Cursor expiry returns `410 Gone` with `WOS-1410`.

## Idempotency

`POST /api/v1/cases/{case_id}/processes/{process_id}/appeals` requires `Idempotency-Key` per ADR 0082 D-16 — filing an appeal is an externally-visible side effect (governance records emit, provenance events fire). A repeat request within the retention window returns the original `Appeal` resource unchanged.

`GET /api/v1/appeals/{urn}` and `GET /api/v1/cases/{case_id}/processes/{process_id}/appeals` are idempotent by construction.
...
- `WOS-1409`: appeal filed against a non-completed process, or appeal state transition attempted from an invalid lifecycle posture.
...
| `_common.schema.json` | `WosResourceUrn` | Appeal and process URNs |
