...
- **Studio.** Studio's `DecisionTable` PolicyObject (per `studio-authoring/specs/binding-and-integration.md`) projects to a `decisionTables[]` entry and a `DecisionTableGuard` on the relevant transition; this replaces the prior chained-FEL-guard workaround.
