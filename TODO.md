...
**Last audited:** 2026-04-19 (session 5 in progress — §4.1 #24a Facts-tier input snapshot, §4.2 #37 Drift Monitor demotion policy binding, and #46 schema-prose enum alignment batch closed; conformance registry now 100 rules / 9 T3)
...
| Fixtures | 41 document + 148 conformance (T3: 0 red / 148 green) |
...
| Lint matrix | 100 rules in `LINT-MATRIX.md` (regenerated from code registries; 197 aspirational → 100 code-registry reality) |
...
> Session 5 in progress 2026-04-19: #37 Drift Monitor demotion policy binding, #46 schema-prose enum alignment, and #24a Facts-tier input snapshot passed local validation after semi-formal review fixes. #24a added executable K-DET-001 conformance coverage; #2 / #5 are now unblocked.

1. **§4.1 #2 Deterministic adverse-decision notice (dual-form)** — Imp 9; now unblocked by #24a. Two prior prereqs (#23 OverrideRecord, NoticeTemplate reconciliation) closed in session 4.
2. **§4.1 #5 Exportable assurance packet for reviewers** — Imp 8; depends on #2 adverse-decision notice and the existing provenance/export foundations.
3. **§5.4 Task 7 — synth-trace JSON Schema + drift test** — small follow-up from session 4 wos-synth scaffold; deferred at scaffold time. Closes the "no published schema for the explain artifact" gap noted in semi-formal review Finding 12.
4. **§4.4 Split release trains** — unblocked since §4.2 close. See [plan](thoughts/plans/2026-04-16-wos-release-trains.md).
5. **§5.5 Synthesis benchmark (`wos-bench`)** — unblocked since §5.4 scaffold. See [plan](thoughts/plans/2026-04-16-wos-synthesis-benchmark.md).
6. **v0 spike Tasks 4-5** — conformance gate + retrospective with plan propagation. Still pending from [plan](thoughts/plans/2026-04-17-wos-synth-v0-spike.md).
7. **Remaining §4.1 critical path** — DRAFTS triage (chore that unblocks #20 typed events).

---

## 2026-04-19 session 5 — §4.2 #37 / #46 closure (in review)

Single-stream closeout of the resumable §4.2 work paused in session 4.

### #37 Drift Monitor demotion policy binding

Added two executable T3 fixtures and expected traces for autonomy demotion behavior: `AI-AUTO-001-escalation-expiry-revocation` and `AI-AUTO-002-drift-alert-demotion`. The fixtures lock the two missing autonomy-lifecycle revocation paths: escalation-expiry demotion emits `autonomyDemotion`, and drift-alert demotion emits `autonomyDemotion` + `driftReclassification` while rerouting through `escalated` to land on human review. `crates/wos-conformance/src/rules.rs` registers both as Tested, `trace_parity.rs` runs both parity and honest-runtime-engagement checks, and `LINT-MATRIX.md` is regenerated to 99 rules / 8 T3.
...
Closed the last prerequisite for #2 / #5: Kernel §8.2.1 requires `transitionTags` plus `caseFileSnapshot` for determination-tagged transitions; the kernel schema has `FactsTierRecord` / `CaseFileSnapshot` `$defs` with a pytest contract; `CaseFileSnapshot` in `wos-core` uses RFC 8785 JCS canonicalization plus SHA-256; and `wos-runtime` now persists the per-transition pre-action case-file snapshot emitted by the evaluator. Added executable T3 rule `K-DET-001` plus `k-det-001-determination-snapshot.json`, asserting transition tags, snapshot value, canonical JSON, and digest.

Validation run locally: `cargo test -p wos-core -p wos-runtime -p wos-conformance` passed after review fixes for JCS canonicalization, per-transition snapshot capture, runtime `transitionTags`, and executable K-DET coverage. Broader schema/lint validation is rerun before final closeout.
