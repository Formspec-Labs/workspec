...
**Last audited:** 2026-05-06 — scout-swarm validated all uncompleted items against HEAD; stale paths/counts/gates corrected. ADR 0082 and ADR 0076 closure history → COMPLETED.md.

## Snapshot

| Health | Value |
|---|---|
| Specs / schemas | 41+17 spec/docs under `specs/` (`specs/api/` adds 17 ADR 0082 docs) · 22 schemas (4 core + 2 sidecars + 16 under `schemas/api/`) · 0 SCHEMA-DOC-001 violations |
...
6. **Multi-step session DAG topology (P2, §21 from API coverage audit)** `[5 / 2 / 4]` (**20**) — schema-only close of the single deferred gap from the 2026-05-06 WOS Runtime API coverage audit ([`wos-api-coverage-findings.md`](wos-api-coverage-findings.md) item #21). Two-phase schema restoration. **Revised 2026-05-06 (scout findings):**
