...
- [ ] **#22a ProvenanceKind tier-typing** `[Imp 5 / Cx 4 / Debt 5]` *(extracted from #22)* — Replace the 93-variant `ProvenanceKind` monolith enum (`crates/wos-core/src/provenance.rs`) with a tier-typed record (kernel / governance / ai / advanced). Load-bearing sub-item of the broader #22 crate split: every new tier-specific variant added to the flat enum ossifies the shape further, so this becomes expensive if it slips. Promoted out of §4.6 because the architectural cost compounds with use; the rest of #22 (directory split, runtime.rs split, CI fence) remains organizational and stays in §4.6.
