...
> Items 1-3 landed 2026-04-18 (see status update under Blockers). Sequence now starts at the §5.3 pair.

1. §5.3 T3 fixture repair (`data.amount` → `caseFile.amount` bridge) — ~2 hours; unblocks honest golden traces.
2. §5.3 runtime instrumentation — ~1-2 days; makes trace a teaching signal.
3. §5.1 reshape pass (`$defs` consolidation for 137 violations) — ~2 hours; high leverage against 901-violation backlog.
