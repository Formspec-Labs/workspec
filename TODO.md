...
**Last audited:** 2026-04-21 (ADR 0059 F3b + Task 5 closed; K-049 LoadBearing).
...
2. **#22a ProvenanceKind tier-typing** `[7 / 5 / 6]` (**42**) — module split / tier typing on `provenance.rs` (unblocks custodyHook wire-format work and keeps provenance changes parallel-safe). **Was** sequenced ahead of F3b; **F3b is now landed** (`bdf7063` post-mutation §10.3 rescan; `f03ca40` K-049 LoadBearing + removal of authored `$continuous` + synthetic trace label `$postMutationRescan`). ADR [0059](thoughts/adr/0059-continuous-mode-post-mutation-rescan.md) **optional follow-ups:** §4.3 Test C (mutation fast-path when no guard reads touched paths); refresh ADR prose to final state. **Gate: none.**
