...
> **2026-04-18 status update:** blockers 2, 3, 4, 5 cleared this session. `SuggestedFix::Custom` now serializes round-trip (`diagnostic.rs:103` struct variant + new `suggested_fix_custom_round_trips` test). `wos-authoring::Command` is `pub(crate)` (lib.rs re-export dropped, enum visibility tightened). Plan doc corrected: `ActorKind::Agent` removed (kernel schema defines only `human | system`; AI agents use `x-wos-ai.agents`; custom actor kinds go through the §10.6 `actorExtension` seam), and `ImpactLevel` variants realigned to the real `RightsImpacting | SafetyImpacting | Operational | Informational` set. T3 fixture repair landed: six happy-path fixtures now carry `initial_case_state` bridges (K-011 determinism + parallel-join, K-020, K-033, K-046, G-030); goldens re-captured from real transitions (5, 1, 2, 1, 7, 12 steps respectively) instead of degenerate empties; six new `happy_path_*` parity tests assert runtime engagement and would have failed before the fix. Blocker 1 (§5.3 teaching signal via `guards_evaluated` / `policies_applied`) still outstanding. **Follow-up**: six fixtures still report `outcome=fail` because their `expected_provenance` entries use legacy keys (`type`/`from`/`to`) instead of serde's `recordKind`/`fromState`/`toState`; a separate cleanup pass will migrate them.
...
> Items 1-4 landed 2026-04-18 (see status update under Blockers). Sequence now starts at §5.3 runtime instrumentation.
...
2. §5.1 reshape pass (`$defs` consolidation for 137 violations) — ~2 hours; high leverage against 901-violation backlog.
3. T3 fixture provenance-field migration — six fixtures use `type`/`from`/`to` instead of `recordKind`/`fromState`/`toState`; once migrated goldens will flip from `outcome: fail` to `outcome: pass`. ~1 hour.
4. Review-warning cleanup across wos-mcp + v0 spike + §4.2 registry comments — batchable, ~3 hours total.
5. §4.2 Task 3 (CI fixture-link test) — ~2 hours; locks in the ratchet.
6. wos-authoring Tasks 4-8 — remaining handlers, undo/redo, WosProject façade, README, integration test; now unblocked.
