...
From the 2026-04-20 semi-formal review of `4fd32e3` (K-049) and `19ad643` (AI-057). Finding 1 (onEntry/onExit) already fixed in `2c6a2e2`; Findings 6, 8, 9 were OBSERVATION-severity style notes and require no action. These four are the remaining WARNING/NIT-severity items worth tracking.

- [ ] **#F2 K-049 indexed/wildcard path asymmetry** `[Imp 3 / Cx 3 / Debt 2]` — `fel_analysis::simple_access_path_string` returns `None` for any path segment that is not `PathSegment::Dot`. Cycles that flow through array members (`caseFile.items[0].x`, `caseFile.items[*].v`) are false negatives because writes retain raw indices but reads get dropped. Options: (a) document as acknowledged false-negative class in the K-049 docstring, or (b) add an index-stripping canonicalizer that normalizes both sides to wildcard-free dotted paths (over-approximates). Option (a) is lower-risk for a warning-tier rule; option (b) is stronger but risks false positives.
- [ ] **#F3 K-049 diagnostic message ↔ runtime `$continuous` drift** `[Imp 4 / Cx 2 / Debt 3]` — K-049's message says "runtime will trip the 100-cycle convergence cap", but the runtime at `crates/wos-core/src/eval.rs:412-421` only re-fires transitions whose event is literally `"$continuous"`. Named-event transitions (the shape used by all 7 K-049 unit fixtures) never get re-evaluated in current runtime, so the rule's warning is technically inaccurate for its own tests. Two sub-choices: (a) reword the message to be spec-faithful ("per Runtime §10.3 re-evaluation could thrash against the 100-cycle cap") and open a separate ADR tracking the spec↔impl divergence, or (b) add a test using `"event": "$continuous"` to exercise the event-shape the runtime actually re-fires. **Not caused by the K-049 commit** — pre-existing drift.
- [ ] **#F4 AI-057 boolean-type check for preconditions** `[Imp 4 / Cx 3 / Debt 2]` — AI-057 only checks FEL parse validity. Spec §3.3.1 requires each precondition entry to evaluate to boolean `true`; a parse-clean entry like `caseFile.amount` or `"string"` always evaluates non-boolean and at runtime skips the capability silently. A static type-inference lint (heuristic: flag top-level bare `FieldRef` / `ContextRef` with no comparison operator) would catch the common authoring mistake. Pair with AG-010 finite-domain reasoning shape.
- [ ] **#F5 `preconditionNotSatisfied` provenance outcome enum enforcement** `[Imp 4 / Cx 2 / Debt 3]` — AI Integration §3.3.1 point 4 mandates `outcome: preconditionNotSatisfied` on the provenance record when a capability is skipped. No schema enum defines the set of permitted provenance `outcome` values; the MUST is unverifiable and any processor could coin a different literal. Options: (a) declare a provenance `outcome` enum in a shared provenance schema (`schemas/kernel/wos-provenance-record.schema.json`) and add `preconditionNotSatisfied` as an entry, or (b) scope down to AI-tier and add the enum in `schemas/ai/wos-ai-integration.schema.json`. **Decision needed:** is provenance `outcome` an AI-tier property or a kernel-tier property?
