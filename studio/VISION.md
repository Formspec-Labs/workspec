...
**Status:** draft — vision
**Date:** 2026-04-30 (relocated 2026-05-02)
**Folder:** `studio/` (originally authored at `studio-authoring/`; moved to `/studio/` once the case-management portal was renamed to `/case-portal/`)
...
## Relationship to the Case Portal (`../case-portal/`)

A sibling product lives at `../case-portal/` (formerly named "WOS Studio" and located at `/studio/`; renamed 2026-05-02 to free this path for the Authoring layer). That product is a **runtime case-management UI** built on React 19 / Vite / Express. It consumes `$wosWorkflow` documents and fixtures and exposes inbox, workflow designer, audit trail, applicant portal, and report-builder surfaces. It is a *reference implementation that runs published workflows*.

This folder, `studio/`, is the **Authoring sibling**. Different audience (program managers, policy/legal/compliance staff, service designers — not case workers). Different problem (transforming source documents and institutional knowledge into reviewed, source-backed, WOS-aligned workflow specifications — not running cases against an already-published workflow). The two products share a name family but have no runtime dependency on each other. See `../case-portal/README.md` for the case-management product.
...
- **Folder name:** originally `studio-authoring/` — chosen over `wos-studio/` to avoid visual collision with the legacy `/studio/` portal of the day. Once that portal was renamed to `/case-portal/` (2026-05-02), Authoring moved to `/studio/` and the disambiguation prefix was dropped.
- **PRD location:** kept inside the product folder (`studio/VISION.md`) following the `crates/wos-server/VISION.md` pattern referenced in the parent `CLAUDE.md` "Read first" table. A short pointer lives at `thoughts/plans/2026-04-30-studio-authoring-product-pointer.md` for discoverability.
...
## Capabilities-first product rule

Studio's user-facing surfaces are workflow concepts — decisions, notices, appeals, service calls, events, authority checks, traceability checks, retention policies — **not** the underlying standards. Standards split into three categories per [`specs/README.md`](specs/README.md) §"External standards (selective reuse)":
- **Backing profiles** Studio recommends and binds: DMN (one-way import), OpenAPI, OPA/Cedar, CloudEvents, JSON-LD ingest. Users author in operational language; Studio infers and proposes the binding.
- **User-visible export formats**: PROV-O (audit export per `authoring-provenance.md`), JSON-LD (workflow rendering); these surface in approval packages and audit reports, not in the authoring picker.
- **Out-of-scope or rejected**: SHACL (rejected as workflow authority), OCEL/XES (out-of-scope; runtime seam is Studio-native), BPMN/CMMN/SCXML (deferred — legacy import only when authored).

The AI-driven binding-recommendation flow is normative:

1. **Detect** — AI identifies a capability need (decision, event, service call, authority check, …) from authoring intent or imported sources.
2. **Propose** — AI creates a typed Studio object (`DecisionRule`, `EventBinding`, `ServiceBinding`, `PolicyEngineBinding`, …).
3. **Ground** — the object links to source citations or reviewer-attested rationale.
4. **Map** — the object maps to WOS concepts via [`specs/studio-to-wos-mapping.md`](specs/studio-to-wos-mapping.md) and an optional backing profile.
5. **Validate** — readiness lint runs against the workspace.
6. **Review** — a human approves, rejects, edits, or requests alternatives.
7. **Compile** — Studio emits WOS artifacts plus optional binding/profile artifacts.
8. **Test** — Studio generates or runs scenarios.
9. **Package** — Studio exports the workflow + decisions + bindings + scenarios + validation reports + approval records.

Studio SHOULD NOT expose backing-profile pickers as the primary authoring UX (no "BPMN vs DMN vs SCXML" affordance). Standard names appear in expert-mode binding inspectors, export packages, implementation handoffs, validation diagnostics, and admin configuration only. The 16 user-facing element kinds in [`specs/workflow-intent.md`](specs/workflow-intent.md) and the four binding kinds in [`specs/binding-and-integration.md`](specs/binding-and-integration.md) (`ServiceBinding`, `EventBinding`, `PolicyEngineBinding`, `DMNImport`) are the catalog Studio exposes. (Companion-PRD reconciliation 2026-05-03; "SHOULD NOT" rather than "MUST NOT" reflects the absence today of an affordance-discipline check — promote to MUST NOT when **either** a UX integration test under `studio/tests/ux/` asserts no backing-profile picker appears in primary authoring surfaces, **or** a workspace-policy field `workspace.policy.affordancePicker = capability-only` lands in `wos-studio-workspace.schema.json` and a corresponding lint rule `WF-LINT-AFFORDANCE-001` enforces it on the workspace document. Either anchor closes the discipline gate; both are out of scope for the G-wave production-polish round.)
...
> **Note (folder adoption):** in this repo the concept model is landed at `studio/CONCEPT-MODEL.md` per the docs-only / everything-in-folder posture documented in this file's preface.
...
> **Note (folder adoption):** in this repo the specs are landed at `studio/specs/<name>.md` per the docs-only / everything-in-folder posture documented in this file's preface.
...
**Landed 2026-05-04** at [`specs/reference-architecture.md`](specs/reference-architecture.md).

The Stage 7 spec ratifies Studio as a **governed
knowledge-to-operational-artifact platform** with multiple
projection targets (WOS workflow + Formspec form co-equal first-
class; decision artifacts, integration bindings, scenario suites,
approval packages, signed export bundles in v1; reports / public
knowledge bases / runtime-observation feedback maximalist). It
composes the 16 existing [`specs/`](specs/) by reference and adds:

- Layer model (Knowledge / Authoring / Design / Validation /
  Publication).
- Component model (cited against the 16 specs).
- Studio-side port catalog (16 ports — disjoint from
  `crates/wos-server`'s port set).
- External-OSS-adapter seam catalog (7 seams:
  `KnowledgeMemoryAdapter`, `DataConnectorAdapter`,
  `MetadataCatalogAdapter`, `LineageAdapter`,
  `DataContractAdapter`, `QualityCheckAdapter`,
  `ProjectionTarget`).
- Projection-target model (uniform `ProjectionTarget` /
  `ExportSink` port; WOS + Formspec co-equal; future targets
  attach the same way).
- Canonical flows (10 named flows).
- Trust / governance invariants (`SA-MUST-arch-060..084`).
- Replay / rebuild contract (`SA-MUST-arch-011`).

Six sibling ADRs decompose the load-bearing decisions:
...
Stage 7 contract code lives in
[`crates/wos-studio-types/src/arch.rs`](crates/wos-studio-types/src/arch.rs)
(port + adapter trait stubs, type aliases — `ConfidenceRecord`,
`AILineageExt`, `ProjectionRef`, `ApprovalPackageRef`,
`ExportBundleRef`); conformance skeleton at
...
**Planned at [`specs/stage-8-vertical-slice.md`](specs/stage-8-vertical-slice.md).**

Width-one path through every layer of the reference architecture.
The slice is deliberately wider than the prior workflow-only
framing — it exercises both first-class projections (WOS workflow
**and** Formspec form) plus a scenario over both:

```text
upload one source (PDF)
→ schema-guided AI extraction (DataRequirement +
   EvidenceRequirement + DecisionRule)
→ human review/approval
→ PolicyKnowledgeMap traceability query
→ projections (Formspec form + WOS workflow step/decision input)
→ scenario over (form data + workflow path)
→ ApprovalPackage + signed ExportBundle
→ filesystem ExportSink
```

Reuses [`examples/snap-redetermination-from-sources/`](examples/snap-redetermination-from-sources/)
as the corpus and reproducibility target. The published
ExportBundle MUST be byte-identical across runs (replay-test
contract per ADR 0087 §2.5).
