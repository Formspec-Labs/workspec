...
- **`SA-MUST-bind-060`** — DMNImport produces an internal DecisionTable PolicyObject; it MUST NOT directly produce `$wosWorkflow` content. The DecisionTable is the canonical Studio representation; the DMN file is the import provenance, retained but not authoritative. *(substrate-pending.)*
- **`SA-MUST-bind-061`** — DMNImport MUST preserve the DMN file as a provenance artifact (referenced via `sourceDmnUri` or inline `dmnXml`); the FEEL→FEL transpilation rationale MUST be queryable for audit. *(substrate-pending; cross-cutting `authoring-provenance.md` audit event catalog: `wos.authoring.dmn-imported`.)*
...
- **`SA-MUST-bind-070`** — Every binding in lifecycleState ≥ `approved` MUST have at least one Scenario in `exercisedByScenarios[]` exercising the binding's happy-path. Bindings without exercising scenarios MUST be flagged as tier-S5 ValidationFindings (`BIND-LINT-030`, "binding-without-contract-test"). (`BIND-LINT-070` enforces.)
- **`SA-MUST-bind-071`** — ServiceBindings that declare `errorHandling.onError != fail-workflow` MUST have at least one additional Scenario exercising the error path. (`BIND-LINT-071` enforces.)
- **`SA-MUST-bind-072`** — PolicyEngineBindings MUST have at least one Scenario exercising both `permit` and `deny` decision outcomes. (`BIND-LINT-072` enforces.)
...
- **`SA-MUST-bind-001`** — Every Binding (Service / Event / PolicyEngine) MUST declare its kernel seam: one of `actorExtension | contractHook | provenanceLayer | lifecycleHook | custodyHook | extensions`. The default mapping by kind:
  ...
  Bindings that need an `x-` extension MUST also carry a `specs/registry/extension-registry.md` entry per [`studio-to-wos-mapping.md`](studio-to-wos-mapping.md) `SA-MUST-map-014`. (`BIND-LINT-001` enforces — workspace.policy.extensionRegistry lookup for every `^x-` body key on every Binding.)
- **`SA-MUST-bind-002`** — Bindings MUST NOT invent new seams. The six canonical seams are the closed set per ADR-0077. (`BIND-LINT-002` enforces.)
...
- **`SA-MUST-bind-011`** — Every `inputBindings[].caseFilePath` MUST resolve to a `CaseFileMapping` in the workspace. Dangling case-file paths MUST be flagged as tier-S3 ValidationFindings. (`BIND-LINT-003` enforces.)
- **`SA-MUST-bind-012`** — Every `outputBindings[].target` MUST resolve to a workspace object (CaseFileMapping, DecisionRule input, EventBinding emission, or ValidationFinding subject). Outputs that don't resolve MUST be flagged as `output-ignored-without-rationale`. (`BIND-LINT-004` enforces — fires on unresolved targets without `ignoredRationale`.)
- **`SA-MUST-bind-013`** — A `ServiceBinding` whose `inputBindings` reference DataElements with `sensitivity ∈ {pii, phi, restricted}` MUST carry `sensitivityHandling` (redaction rules + retention) — or a documented waiver. (`BIND-LINT-005` enforces.)
- **`SA-MUST-bind-014`** — A `ServiceBinding` MUST declare `errorHandling.onError` (one of `retry | fallback | fail-workflow | alert`). Bindings without explicit error handling MUST be flagged as tier-S3 ValidationFindings (`BIND-LINT-005`). (`BIND-LINT-006` enforces.)
- **`SA-MUST-bind-015`** — When `operationKind = arazzo`, the binding's `sequencePosition` MUST be set, and the workflow MUST tolerate pause/resume across the sequence boundary (per `MD-INVENTORY.md` Arazzo binding spec). *(substrate-pending.)*

### EventBinding integrity

- **`SA-MUST-bind-020`** — Every `EventBinding` MUST carry `cloudEventsExtensions.woscausationeventid` and `woscorrelationkey`. These are not optional — WOS uses them for case correlation.
- **`SA-MUST-bind-021`** — When `direction = consumed`, the binding MUST identify a source/system that emits the event. Consumed events without a source MUST be flagged as tier-S3 ValidationFindings (`BIND-LINT-010`). (`BIND-LINT-010` enforces.)
- **`SA-MUST-bind-022`** — When `direction = emitted`, the binding MUST identify a recipient/channel. Emitted events without a recipient MUST be flagged as tier-S3 ValidationFindings (`BIND-LINT-011`). (`BIND-LINT-011` enforces.)
...
- **`SA-MUST-bind-024`** — Every `payloadShape` field of `sensitivity ∈ {pii, phi, restricted}` MUST appear with a redaction rule when emitted (preventing PII leakage in event channels). (`BIND-LINT-012` enforces.)

### PolicyEngineBinding integrity

- **`SA-MUST-bind-030`** — Every `PolicyEngineBinding` MUST normalize the engine's response into `{decision, reasons[], obligations[]}` per the WOS PolicyDecision contract (`WOS-IMPLEMENTATION-STATUS.md`). Raw engine responses MUST NOT bleed into the workflow. *(substrate-pending: response-normalization is a transformation, not a structural shape.)*
- **`SA-MUST-bind-031`** — `composition` MUST be `deny-overrides` (the only allowed value in v1; per `specs/ai/ai-integration.md §4.6`). Other composition modes are deferred.
- **`SA-MUST-bind-032`** — The binding's `inputContract.caseFilePaths[]` MUST be a complete declaration of every case field the engine reads. Engines that read undeclared fields create privacy/audit hazards; this declaration is what makes the binding auditable. (`BIND-LINT-020` enforces the tractable lint slice — non-empty caseFilePaths declaration; full per-engine completeness check stays runtime-pending.) *(substrate-pending: per-engine completeness verification.)*
- **`SA-MUST-bind-033`** — `outputNormalization.reasonsMapping` MUST translate engine reason codes into reviewer-readable plain language. Bindings with unmapped reason codes MUST be flagged as tier-S3 ValidationFindings. (`BIND-LINT-021` enforces.)

### DecisionTable integrity

- **`SA-MUST-bind-040`** — Every `DecisionTable` MUST declare `hitPolicy` and `completenessRequirement`. Tables without explicit hit policy or completeness MUST be flagged as tier-S2 ValidationFindings (`POM-LINT-009`).
...
- **`SA-MUST-bind-044`** — DecisionTables MUST project to a parent-kernel `decisionTables[*]` entry preserving the input/output declarations, row ids, hit policy, and per-row rationale; the referencing transition MUST emit a `DecisionTableGuard` of form `{kind: "decisionTable", ref, outputColumn, inputBindings}` that resolves under parent Kernel §4.5.1 (lint K-051). *(substrate-pending: compiler in [`compiler-contract.md`](compiler-contract.md).)*

### Decision tables and adverse outcomes

- **`SA-MUST-bind-050`** — A DecisionTable whose outputs include an Outcome with `polarity = adverse` AND `triggersDueProcess = true` MUST satisfy the same notice/appeal linkage rules as any other DecisionRule. (Cross-cutting `SA-MUST-pom-030`.) (`WF-LINT-001` enforces — works on every adverse Outcome regardless of source.)
...
### Binding Inspector (read-only aggregation surface)

A read-only product surface that aggregates per-workspace binding state for technical and semi-technical users. The data already exists across the workspace — this section names the aggregation surface so future UX work has a spec anchor. Per the capabilities-first product rule (`VISION.md §"Capabilities-first product rule"`), default authoring views show user-facing capability names (decision / event / service-call); the Binding Inspector is where standard names (DMN, OpenAPI, CloudEvents, OPA) become visible.

For each binding the inspector aggregates:

- The user-facing capability name + Studio object kind (`ServiceBinding`, `EventBinding`, `PolicyEngineBinding`, `DMNImport`). DecisionRule (a `policy-object-model.md` kind) is NOT a binding kind; the inspector composes its data into the workflow-level Decision view, not the Binding Inspector proper.
- The seam this binding attaches at (one of the six canonical kernel seams per ADR 0077).
- Source citations from the underlying PolicyObject(s).
- WOS-side projection target — kind-specific per the WOS mappings table in §"WOS mappings": ServiceBinding / EventBinding / PolicyEngineBinding project to `$.integration.bindings[*]`; DecisionTable (when surfaced via DMNImport) projects to `$.decisionTables[*]` plus `$.lifecycle.transitions[*].guard`; DMNImport itself does NOT directly produce `$wosWorkflow` content (per `SA-MUST-bind-060`) — it transpiles to internal Studio DecisionTable PolicyObjects.
- Selected backing profile (OpenAPI operation, Arazzo sequence, CloudEvents extension, OPA/Cedar/XACML engine, DMN decision id) — including the `composition` field for PolicyEngineBindings (per SA-MUST-bind-031).
- Input / output bindings (caseFile path → request path mappings).
- Mapping state from `studio-to-wos-mapping.md` (`mapsToWos` / `requiresSpecExtension` / `unmappedButApproved` / `authoringOnly`) plus any `ExtensionRecord` linkage.
- Readiness tier(s) the binding fires on (typically S3 + S4 + S6).
- Active validation findings + their lifecycleState.
- Scenario coverage from `exercisedByScenarios[]` + per-scenario pass/fail status.
- Approval-package status + reviewer attestation chain.

The inspector is not authoritative for any of these fields — it composes them by reading the underlying objects. Edits flow back to the underlying PolicyObject / Mapping / Scenario / ApprovalPackage. (Companion-PRD reconciliation 2026-05-03.)
