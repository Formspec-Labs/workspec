...
- **`ingestFormat`** — how the version was ingested: `pdf | html | docx | markdown | json-ld | rdf-xml | turtle | akoma-ntoso`. Determines parser path; `json-ld` invokes the JSON-LD ingest pipeline (see §"JSON-LD source ingest" below); `akoma-ntoso` invokes the Akoma Ntoso ingest pipeline (see §"Akoma Ntoso ingest path" below).
...
- **`SA-MUST-source-001`** — A SourceDocument MUST have at least one SourceVersion before any extraction or citation operation may target it. (`SV-LINT-007` enforces.)
- **`SA-MUST-source-002`** — Every SourceVersion MUST progress through `uploaded → parsed → indexed → classified` before it can be promoted to `current`, `preliminary`, or `disputed`. (`SV-LINT-008` enforces the tractable lint slice — presence of `parsingResult` on any version downstream of `uploaded`; full temporal-progression history check stays runtime-pending.) *(substrate-pending: full progression history.)*
- **`SA-MUST-source-003`** — `parsingResult.status = ok` MUST be required for a SourceVersion to leave `uploaded`. A SourceVersion with `parsingResult.status = partial` MAY proceed to `indexed` only if a reviewer explicitly waives the parsing warnings; the waiver MUST be recorded as a ReviewerResolution. (`SV-LINT-009` enforces — `parsingResult.status='ok'` required to leave `uploaded`; `partial` requires `parsingWaiverRef`.)
- **`SA-MUST-source-004`** — Classification MUST set `effectiveStart` for any SourceVersion that will become `current` or `preliminary`. A SourceVersion without `effectiveStart` MUST NOT be promoted to `current`. (`SV-LINT-004` enforces.)
- **`SA-MUST-source-005`** — Within a single SourceDocument, at most one SourceVersion MAY hold the `current` lifecycle state at any given moment. Promoting a new version to `current` MUST atomically transition the prior `current` version to `superseded`. (`SV-LINT-010` enforces the cardinality slice — at most one SourceVersion at lifecycleState=`current` per SourceDocument.) *(substrate-pending: atomic-transition semantics on supersession.)*

### Sections + pageability

- **`SA-MUST-source-010`** — Every SourceSection MUST carry an anchor that is unique within its SourceVersion. (`SV-LINT-005` enforces.)
- **`SA-MUST-source-011`** — When a SourceVersion's `pageable` flag is `true`, every SourceSection in that version MUST carry a `pageRange`. (`SV-LINT-011` enforces.)
- **`SA-MUST-source-012`** — A SourceSection's `siblingOrder` MUST be stable across re-parsings of the same SourceVersion's payload — i.e., re-running the parser on identical bytes MUST produce the same section sequence. *(substrate-pending: requires the SourceVersion parser, which is a Stage-7/8 ingestion-tier component — no parser exists in `studio/crates/` today.)*
...
- **`SA-MUST-source-020`** — Every SourceCitation MUST resolve to a real SourceSection inside an existing SourceVersion. Dangling citations (the section was deleted or the version was rebuilt with different anchors) MUST be flagged as tier-S1 ValidationFindings on the citing object. (`SV-LINT-001` enforces.)
- **`SA-MUST-source-021`** — Every SourceCitation MUST carry an `excerpt`. The excerpt MUST be a substring of the cited SourceSection's `text` (Unicode-normalized NFC). If the section's text changes (e.g., a new SourceVersion is uploaded and the section's text drifts), the citation's excerpt MUST be re-verified before any object that depends on it can advance its lifecycle. (`SV-LINT-002` enforces the excerpt-substring check; *runtime-pending: re-verification on supersession.*)
- **`SA-MUST-source-022`** — A SourceCitation's `relation` MUST be one of `supports | derivedFrom | conflictsWith | supersedes | contradicts`. Other relation strings MUST be rejected at citation creation.
...
- **`SA-MUST-source-030`** — Promoting a new SourceVersion to `current` MUST trigger a supersession event recording: `{priorVersionId, newVersionId, supersededAt, supersededBy}`. *(substrate-pending: event emission on lifecycle transition.)*
- **`SA-MUST-source-031`** — Supersession MUST trigger a ChangeImpactReport (see [`change-impact.md`](change-impact.md)) enumerating every PolicyObject, Mapping, Scenario, and PublishedWorkflowPackage that cites the prior `current` version. *(substrate-pending; cross-spec coupling with change-impact.)*
...
- **`SA-MUST-source-033`** — A SourceVersion in `disputed` lifecycle state MUST NOT be the sole basis for any approved PolicyObject. An approved PolicyObject citing only disputed sources MUST surface a tier-S1 ValidationFinding until the dispute is resolved. (`SV-LINT-003` enforces — workspace rule covers `disputed | superseded` sole-basis cases.)
...
- **`SA-MUST-source-050`** — A SourceVersion with `ingestFormat = json-ld` MUST preserve the source's `@context` and IRI structure in the indexed form. Reviewers MAY view both the JSON-LD-native form and the human-readable rendering. *(substrate-pending.)*
- **`SA-MUST-source-051`** — A SourceVersion ingested as JSON-LD MUST carry `canonicalSourceRef.referencedUri` resolving to the source's canonical IRI (e.g., `https://www.ecfr.gov/api/renderer/v1/title-7/section-273.10`). The `@id` IRIs of nested objects in the JSON-LD MAY be cited directly via SourceCitation `relation: derivedFrom`. (`SourceVersion` schema with `ingestFormat=json-ld` requires `canonicalSourceRef.referencedUri` via if/then; live in `wos-studio-source.schema.json`.) *(substrate-pending: re-fetch + IRI resolution.)*
- **`SA-MUST-source-052`** — When a JSON-LD-ingested source is re-fetched and the `@context` differs from the prior version, supersession event MUST trigger AND a tier-S1 ValidationFinding MUST surface (`SOURCE-LINT-006`, "json-ld-context-drift") so reviewers verify semantic continuity. (`SV-LINT-012` enforces.)

### Akoma Ntoso ingest path

Some legal-source publishers (e.g., national legislatures publishing under the OASIS LegalDocML / Akoma Ntoso family) ship structured legal XML carrying section-level granularity, effective-date metadata, supersession links, and authority attribution. Studio supports Akoma Ntoso as a first-class ingest path parallel to JSON-LD because: (a) the document tree's `<section>` / `<article>` / `<paragraph>` boundaries map naturally to SourceSection anchors; (b) `<eId>` attributes provide stable structural references that survive reformatting; (c) effective-date metadata in `<FRBRdate>` populates `effectiveStart` / `effectiveEnd` directly; (d) `<note type="affectedBy">` + `<ref>` capture supersession edges across the source corpus.

- **`SA-MUST-source-080`** — A SourceVersion with `ingestFormat = akoma-ntoso` MUST preserve the document's `<eId>` structural hierarchy as SourceSection anchors. Anchors take the form `eId:<encoded-eid>` (e.g., `eId:sec_273-10__para_1`). *(substrate-pending: parser; lint-pending: anchor-stability check.)*
- **`SA-MUST-source-081`** — A SourceVersion with `ingestFormat = akoma-ntoso` MUST extract `<FRBRdate>` values into `effectiveStart` / `effectiveEnd` on the version envelope. Sources without `<FRBRdate>` MUST be flagged as a tier-S1 ValidationFinding ("akoma-ntoso-missing-frbr-date") so reviewers supply the missing temporal scope. The `SV-LINT-*` rule id is minted when the rule lands in the registry. (`SV-LINT-013` enforces.)
- **`SA-MUST-source-082`** — Akoma Ntoso `<note type="affectedBy">` references MUST project to cross-document supersession edges (per `SA-MUST-source-006/007`). The supersession trigger is the `<ref>` href; resolution is reviewer-driven, identical to non-Akoma-Ntoso supersession flows. *(substrate-pending.)*

The `ingestFormat` enum on SourceVersion (per §"Data model" above) gains `akoma-ntoso` as an admitted value. Generic legal-source ingestion (PDF / HTML / DOCX / Markdown for sources that don't ship in structured legal XML) remains the default path; Akoma Ntoso is an opt-in upgrade for publishers that supply it. (Companion-PRD reconciliation 2026-05-03.)
...
- **`SA-MUST-source-060`** — When a SourceVersion has `len(contentLocales) > 1`, every SourceSection MUST carry text for the FIRST (authoritative) locale. Other locales MAY be missing per-section; missing translations MUST be flagged but do NOT block ingest. (`SV-LINT-014` enforces.)
- **`SA-MUST-source-061`** — A SourceCitation that quotes a non-authoritative translation MUST carry `excerptLocale` AND a back-reference to the authoritative-locale equivalent excerpt. Reviewer-driven equivalence MUST be attested via ReviewerResolution if the translation is contested. *(substrate-pending.)*
