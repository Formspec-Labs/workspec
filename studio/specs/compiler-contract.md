...
- **`SA-MUST-cmp-001`** — Compilation MUST be **deterministic**: given identical compile-inputs (frozen at a specific moment, including identical compiler version, identical schema version, identical workspace state, identical referenced objects), the output `$wosWorkflow` MUST be identical byte-for-byte modulo JSON key ordering. (Covered by `studio/crates/wos-studio-compiler/tests/determinism.rs::cross_process_compile_byte_identical` for cross-process drift, plus `studio/crates/wos-studio-compiler/src/pipeline.rs::deterministic_compile_byte_identical_in_process_repeats` for in-process repetition.)
- **`SA-MUST-cmp-002`** — JSON key ordering in the output MAY vary; structural identity (semantic equality) is the determinism boundary. The compile manifest's `sourceVersionsConsumed[]` and `policyObjectsConsumed[]` MUST be sorted deterministically (e.g., by id) so audit comparisons are stable. *(substrate-pending.)*
- **`SA-MUST-cmp-003`** — The compiler MUST NOT introduce non-deterministic content (timestamps, UUIDs, random ordering) into the artifact. Timestamps in the manifest are compile-time; timestamps in the artifact body (e.g., effective dates) come from the input PolicyObjects. (Covered by `studio/crates/wos-studio-compiler/tests/determinism.rs::cross_process_compile_byte_identical` — any UUID / random-order leak would surface as cross-process drift in `wos-workflow.json` or `manifestHash`.)
- **`SA-MUST-cmp-004`** — When two PolicyObjects produce equivalent artifact content (e.g., two NoticeRequirements compiling to overlapping `governance.notices[*]` entries), the compiler MUST detect the collision at phase 4 and halt with `failureKind = artifact-collision`. AI / reviewer judgment MUST resolve the collision; the compiler does NOT auto-merge. *(substrate-pending; cross-cutting [`studio-to-wos-mapping.md`](studio-to-wos-mapping.md) `mapping-target-collision`.)*

### Input completeness

- **`SA-MUST-cmp-010`** — Phase 1 (Select approved policy objects) MUST verify that every `policyObjectRefs[]` entry of the WorkflowIntent's elements is in lifecycle state `approved` or later. Missing approvals halt the pipeline with `failureKind = unapproved-input`. *(substrate-pending.)*
- **`SA-MUST-cmp-011`** — Phase 2 (Resolve mapping records) MUST verify that every PolicyObject has exactly one mapping. PolicyObjects without a mapping halt the pipeline with `failureKind = unmapped-input`. *(substrate-pending; cross-cutting `SA-MUST-map-001`.)*
- **`SA-MUST-cmp-012`** — Phase 3 (Generate workflow intent) MUST verify that every WorkflowElement's `bridge` is well-formed for its `kind`. Malformed bridges halt with `failureKind = malformed-bridge`. *(substrate-pending; cross-cutting [`workflow-intent.md`](workflow-intent.md) `SA-MUST-wfi-040`.)*
- **`SA-MUST-cmp-013`** — When the WorkflowIntent references events for which no `EventBinding` exists, phase 4 MUST halt with `failureKind = unresolved-event-reference`. (Cross-cutting [`binding-and-integration.md`](binding-and-integration.md) `SA-MUST-bind-023`.) *(substrate-pending.)*
- **`SA-MUST-cmp-014`** — When the WorkflowIntent references API operations for which no `ServiceBinding` covers required inputs, phase 4 MUST halt with `failureKind = incomplete-service-binding`. *(substrate-pending.)*

### Emission rules

- **`SA-MUST-cmp-020`** — Phase 4 emits `$wosWorkflow` content; only `mapsToWos` PolicyObjects (and their bindings) project into the artifact body. `authoringOnly`, `requiresSpecExtension` (without an `x-` target), and `unmappedButApproved` PolicyObjects MUST NOT produce artifact body content. *(substrate-pending; cross-cutting `SA-MUST-map-030`, `SA-MUST-map-040`.)*
- **`SA-MUST-cmp-021`** — `requiresSpecExtension` PolicyObjects whose ExtensionRecord is `lifecycleState = shipped` AND has an `x-` target MUST emit content under that `x-` key. ExtensionRecords still `open` MUST halt the pipeline if their host PolicyObject is workflow-bearing. (Cross-cutting `SA-MUST-map-005`.) *(substrate-pending.)*
- **`SA-MUST-cmp-022`** — Phase 5 emits `wos-tooling.scenarios[*]` entries. Only Scenarios in lifecycle state ≥ `reviewed` are emitted. Scenarios in `failing` or `acceptedAsKnownGap` are emitted with a status flag (per `SA-MUST-scn-023`) so WOS conformance does not treat them as expected-passing. *(substrate-pending.)*
- **`SA-MUST-cmp-023`** — `unmappedButApproved` mappings MUST be enumerated in the published artifact's release notes per `SA-MUST-map-042`. *(substrate-pending.)*
- **`SA-MUST-cmp-024`** — Authoring provenance MUST be projected per [`authoring-provenance.md`](authoring-provenance.md) `SA-MUST-prov-030` / `SA-MUST-prov-031` / `SA-MUST-prov-032`. The compiler is the sole emitter of compact projections. *(substrate-pending.)*
...
- **`SA-MUST-cmp-030`** — Phase 7 MUST run three external gates **in order**, in this exact sequence, each producing a named pass/fail result. Subsequent gates MUST NOT run if a prior gate fails:
  ...
  Each gate's result is recorded in the compile manifest. *(substrate-pending; cross-cutting [`readiness-validation.md`](readiness-validation.md) tier S6.)*
- **`SA-MUST-cmp-031`** — Failure of any external gate produces a tier-S6 ValidationFinding lifted into the workspace per [`readiness-validation.md`](readiness-validation.md) §"WOS mappings". The artifact is NOT published; the workflow's `lifecycleState` does NOT advance. *(substrate-pending.)*
...
- **`SA-MUST-cmp-040`** — Compilation has exactly three failure dispositions:
  ...
  Implementations MUST NOT introduce intermediate dispositions. *(substrate-pending: no compile-manifest schema exists today; the disposition enum lives at the Rust enum surface.)*
...
- **`SA-MUST-cmp-050`** — Every compile manifest MUST record `compilerVersion`, `schemaVersion` (the version of `wos-workflow.schema.json`), `wosVersionPin` (claim string per parent RELEASE-STREAMS.md), and the `workflowIntentVersion` consumed. *(substrate-pending: no compile-manifest schema exists today; required-field check lives on the Rust struct.)*
- **`SA-MUST-cmp-051`** — A compiler version bump MAY change the artifact byte-for-byte (e.g., new optional fields populated, deprecated fields cleaned up); semantic equality with the prior compiler version's output for the same inputs MUST be preserved. The existing `cross_process_compile_byte_identical` determinism harness at `crates/wos-studio-compiler/tests/determinism.rs` covers byte-equality within a single compiler version; the cross-version semantic-equality fixture lands when a v2 compiler exists to compare against (substrate dependency on compiler v2). *(substrate-pending: cross-version comparison harness.)*
- **`SA-MUST-cmp-052`** — A WorkflowIntent's `wosVersionPin` constrains compilation: the compiler MUST refuse to compile against schema/stream versions outside the pin. Updating the pin is an explicit reviewer action per `change-impact.md` `triggerKind = wos-version-deprecation`. The pin's typed kernel shape is `wos-workflow.schema.json#/$defs/WosVersionPin` (`{envelopeVersion: "X.Y", includedBlocks?: enum[]}`); landed F1.3 (2026-05-02). *(lint-pending: pin enforcement is a publication-gate readiness check, not a runtime execution detail.)*
...
- **`SA-MUST-cmp-054`** — When a parent stream version reaches deprecation per parent `COMPATIBILITY-MATRIX.md`, the compiler MUST surface a tier-S6 ValidationFinding `CMP-LINT-010` "wos-version-deprecation-pending" 90 days before the deprecation effective date, prompting reviewer-driven migration. (`CMP-LINT-010` enforces.)

### Reproducibility / disaster-recovery

- **`SA-MUST-cmp-060`** — Given the compile manifest, the workspace state at the recorded time, and the recorded compiler/schema/wosVersionPin versions, the same compile MUST be reproducible. (Covered by `studio/crates/wos-studio-compiler/tests/determinism.rs::cross_process_compile_byte_identical`, which spawns the compiled binary across separate processes and asserts byte-identical `wos-workflow.json` + stable `manifestHash` — the reproducibility primitive.)
- **`SA-MUST-cmp-061`** — The compile manifest MUST be small enough to be embedded in the published artifact. (Manifest details that exceed embedding capacity are stored in the workspace export per [`authoring-provenance.md`](authoring-provenance.md) `SA-SHOULD-prov-034`.) *(substrate-pending: size-budget invariant is operational, not structural.)*
- **`SA-MUST-cmp-062`** — Workspace-export-driven disaster recovery: given a workspace export bundle (sources + PolicyObjects + mappings + scenarios + provenance log + compile manifest + custody-anchored receipts per [`authoring-provenance.md`](authoring-provenance.md) `SA-MUST-prov-081`) and the recorded compiler version, an external party MUST be able to reproduce the published artifact byte-for-byte (modulo JSON key order). The workspace export is the disaster-recovery primitive. *(substrate-pending.)*
- **`SA-MUST-cmp-063`** — The workspace export bundle MUST be self-contained: every external reference (parent schemas, parent custody receipts) MUST either (a) be inlined, or (b) carry an externally-resolvable URI + content hash. *(substrate-pending: bundle-completeness check is the export builder's contract; no static schema captures "either inlined or hash-bound".)*
...
- **`SA-MUST-cmp-070`** — Compiler-phase transitions MUST emit `wos.compiler.phase-started` / `wos.compiler.phase-completed` / `wos.compiler.phase-halted` events with phase identifier and outcome. *(substrate-pending; coordination-pending parent PLN-0384.)*
- **`SA-MUST-cmp-071`** — External gate results (schema-pass / lint-pass / conformance-pass) MUST emit `wos.compiler.gate-passed` / `wos.compiler.gate-failed` with gate name and finding refs. *(substrate-pending.)*
- **`SA-MUST-cmp-072`** — Compile success / failure MUST emit `wos.compiler.compile-succeeded` (with manifest ref) / `wos.compiler.compile-failed` (with failureKind). *(substrate-pending.)*
- **`SA-MUST-cmp-073`** — Compiler events MUST custody-anchor per [`authoring-provenance.md`](authoring-provenance.md) `SA-MUST-prov-081`. The compile manifest's `manifestHash` and the ApprovalPackage's binding to the artifact derive their authenticity from this anchor chain. *(substrate-pending.)*
