...
- **`SA-MUST-map-001`** — Every PolicyObject MUST have exactly one StudioToWosMapping record once its `lifecycleState` reaches `approved`. Approval without a mapping declaration MUST be rejected. (`MAP-LINT-001` enforces.)
- **`SA-MUST-map-002`** — `mappingState` MUST be exactly one of the four values; multiple-state declarations MUST be rejected.
- **`SA-MUST-map-003`** — When `mappingState = mapsToWos`, the record MUST carry a non-empty `targets[]`. When `mappingState = requiresSpecExtension`, it MUST carry an `extensionRecordRef`. When `mappingState = unmappedButApproved`, it MUST carry an `unmappedRationale` of at least 50 characters (no one-line "TBD"-style rationales). When `mappingState = authoringOnly`, it MUST NOT carry any of the three. (Already encoded in `wos-studio-mapping.schema.json` via the four-way if/then on `mappingState`: mapsToWos→targets[]≥1; requiresSpecExtension→extensionRecordRef OR extensionRecord; unmappedButApproved→unmappedRationale minLength 50; authoringOnly→none.)
- **`SA-MUST-map-004`** — A WorkflowIntent MUST NOT advance from `mapped → validationReady` while any approved PolicyObject it references has `mappingState = unmappedButApproved` without a workflow-level reviewer override. (`MAP-LINT-009` enforces.)
- **`SA-MUST-map-005`** — A WorkflowIntent MUST NOT advance from `validationReady → scenarioTested` while any approved PolicyObject it references has `mappingState = requiresSpecExtension` whose corresponding ExtensionRecord is in `lifecycleState = open` (i.e., not yet shipped in WOS). (`MAP-LINT-010` enforces.)
...
- **`SA-MUST-map-014`** — A `Target` whose `wosJsonPath` includes path segments under `x-` extension keys MUST also carry a reference to an ExtensionRecord (or to an `extensions` registry entry per [`../../specs/registry/extension-registry.md`](../../specs/registry/extension-registry.md)). Bare `x-` targets without an extension audit trail MUST be rejected. (`MAP-LINT-008` enforces.)

### Extension records

- **`SA-MUST-map-020`** — An ExtensionRecord MUST identify exactly one of the **six canonical kernel seams** (per [`../../CLAUDE.md`](../../CLAUDE.md): `actorExtension`, `contractHook`, `provenanceLayer`, `lifecycleHook`, `custodyHook`, `extensions` / `x-` keys) as the proposed attachment point. (Cross-cutting with [ADR 0077](../../thoughts/adr/0077-named-seams-invariant.md).)
- **`SA-MUST-map-021`** — An ExtensionRecord MUST carry at least one `motivatingPolicyObjectRef` — the Studio concept(s) that lack a current WOS counterpart. Speculative extensions ("we might need this someday") MUST be rejected at extension creation. (`MAP-LINT-011` enforces.)
- **`SA-MUST-map-022`** — `proposedSemantics` MUST be substantive prose (at least 200 characters); `proposedSchemaSketch` MUST be a non-empty informal sketch. Empty or one-line entries MUST be rejected.
- **`SA-MUST-map-023`** — When an ExtensionRecord is `approved` and the corresponding upstream WOS spec extension lands, the implementation MUST: (a) mark the ExtensionRecord `lifecycleState = shipped`; (b) prompt reviewers to migrate the host PolicyObject's mapping from `requiresSpecExtension` to `mapsToWos` with the now-existing target. The migration is reviewer-driven, not automatic. *(substrate-pending; cross-cutting with the upstream extension PR cycle.)*
...
|---|---|---|---|
| `ApplicabilityScope` PolicyObject | **LANDED 2026-05-02** as kernel `wos-workflow.schema.json#/$defs/ApplicabilityScope`. Studio compilation projects the Effectiveness object's jurisdictions/dateRanges/caseFilters/effectivenessRef into the typed shape; downstream `governance.applicabilityScope` consumers $ref the kernel def. | n/a (F1.1; queued; landed) |
| `EffectivePeriod` PolicyObject | **LANDED 2026-05-02** as kernel `wos-workflow.schema.json#/$defs/EffectivePeriod`. Studio compilation projects Effectiveness temporal scope (intervals + retroactiveFrom + sunsetAt) into the typed shape. | n/a (F1.2; queued; landed) |
| `DecisionTable` (rows) | **LANDED 2026-05-01** as parent Kernel §4.5.1 (`decisionTables[*]` + `DecisionTableGuard`). Studio DecisionTable now `mapsToWos` → `decisionTables[*]` with row-id preserved. Row-level scenario coverage references the projected row id directly. | n/a (was queued; landed) |
...
| `governance.deonticConstraints[*]` LegalRuleML | **LANDED 2026-05-02** — kernel `wos-workflow.schema.json#/$defs/DeonticConstraint` expanded to carry `id`, `actor`, `condition`, `defeasible`, `citationRefs[]`, and `x-legalruleml-iri` alongside the existing `kind` + `expression` per OASIS LegalRuleML §3. Studio's `agents[].deonticConstraints` and any future `governance.deonticConstraints` $ref the typed shape. | n/a (F1.4; queued; landed) |
| `caseFile.fields[*].canonicalTermRef` and `sensitivity` (DPV) | **LANDED 2026-05-02** — kernel `FieldDeclaration` now carries optional `canonicalTermRef` + `dpvSensitivity` properties. DataElement annotation per `terminology-and-canonical-vocabulary.md` no longer requires `x-` extension keys. | n/a (F1.5; queued; landed) |
| Workflow-level `wosVersionPin` | **LANDED 2026-05-02** — kernel `wos-workflow.schema.json#/$defs/WosVersionPin` + top-level optional `wosVersionPin` property. Replaces the free-form claims-string in `RELEASE-STREAMS.md` with a typed `{envelopeVersion, includedBlocks[]}` shape. Compiler refusal logic per `SA-MUST-cmp-052`. | n/a (F1.3; queued; landed) |
| `custody` block conditional | **LANDED 2026-05-02** — schema `allOf` now requires `custody` for `rights-impacting` and `safety-impacting` workflows (mirrors the existing governance + signature conditionals; ADR-0076). Previously delegated to lint. | n/a (F1.6; landed) |
...
- **`SA-MUST-map-030`** — When `mappingState = authoringOnly`, the implementation MUST NOT emit the host PolicyObject into the compiled `$wosWorkflow` artifact's content. *(substrate-pending: compiler enforcement.)*
- **`SA-MUST-map-031`** — Authoring-only PolicyObjects MAY still contribute to the published artifact's authoring-provenance projection (per [`authoring-provenance.md`](authoring-provenance.md) `SA-MUST-prov-030`); this is a *projection*, not the object itself. *(substrate-pending.)*
...
- **`SA-MUST-map-040`** — `mappingState = unmappedButApproved` MUST be approved by a reviewer in a role authorized to take this action (workspace policy decides; default: workflow owner role). *(substrate-pending: role-policy.)*
- **`SA-MUST-map-041`** — Each `unmappedButApproved` mapping MUST surface as a tier-S3 `noisy-unmapped` ValidationFinding for the duration of the host PolicyObject's lifetime — the finding is informational (severity `warn`), not blocking, but it is **always visible** to ensure publication-time review. (`MAP-LINT-004` enforces.)
- **`SA-MUST-map-042`** — A workspace's published artifacts MUST list every `unmappedButApproved` mapping in the published artifact's release notes (PRD §9.9 approval package). *(substrate-pending: compiler emission of release notes.)*
...
- **`SA-MUST-map-050`** — Mapping state changes MUST emit AuthoringProvenanceRecords with `eventKind = mapped` or `mappingStateAssigned` (per [`authoring-provenance.md`](authoring-provenance.md) `SA-MUST-prov-001`). *(substrate-pending.)*
- **`SA-MUST-map-051`** — Source supersession that demotes a host PolicyObject (per [`policy-object-model.md`](policy-object-model.md) `SA-MUST-pom-022`) MUST also demote its mapping back to `draft` so the mapping is re-reviewed against the new citation context. *(substrate-pending.)*
...
| DecisionTable (DecisionRule with `body.form: "table"`) | `mapsToWos` | [`../../specs/kernel/spec.md`](../../specs/kernel/spec.md) §4.5.1 | `$.decisionTables[*]` (table catalog) + `$.lifecycle.transitions[*].guard` of form `{kind: "decisionTable", ref, outputColumn, inputBindings}`. Row id, hit policy, and rationale preserved through projection. |
