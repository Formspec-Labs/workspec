...
/**
 * Optional JSON Schema URI enabling editor validation and autocompletion. When present, editors (VS Code, IntelliJ, etc.) validate the document against this schema. Omit in production documents; the authoritative schema URI is derived from the document type marker (e.g., '$wosOntologyAlignment': '1.0'). Used for author-time tooling only — runtime processors MUST ignore this field.
 */
export type JsonSchemaUri = string;

/**
 * A WOS Ontology Alignment sidecar document. Renamed from the prior wos-semantic-profile per ADR 0076 D-3. Declares linked-data semantics for a WOS workflow without altering core processing: a JSON-LD @context mapping WOS properties to RDF terms, SHACL shape references for semantic validation, PROV-O vocabulary mappings for provenance export, and XES/OCEL configuration for process mining interoperability. Per ADR 0076 D-3, transition-tag vocabulary returns to the kernel/spec.md and is intentionally NOT part of this sidecar; only ontology export concerns remain. Adds interpretation and export capability — never changes how WOS documents are processed at runtime.
 */
export interface WOSOntologyAlignmentSidecar {
  /**
   * WOS Ontology Alignment specification version. MUST be '1.0'. Identifies this document as a WOS Ontology Alignment sidecar (formerly wos-semantic-profile) and pins the specification version. Processors reject documents with unsupported versions.
   */
  $wosOntologyAlignment: '1.0';
  $schema?: JsonSchemaUri;
  targetWorkflow: TargetWorkflow;
  /**
   * Version of this Ontology Alignment Document. SemVer is RECOMMENDED.
   */
  version?: string;
  /**
   * Human-readable name for this ontology alignment. Displayed in tooling and provenance exports when identifying the linked-data layer for this workflow.
   */
  title?: string;
  /**
   * Human-readable description of the ontology alignment's purpose and scope. Documents which vocabularies, shape graphs, and export formats are configured for this deployment.
   */
  description?: string;
...
/**
 * The WOS Kernel Document this ontology alignment targets. Specifies URL and compatible version range. The ontology processor resolves the JSON-LD context, SHACL shapes, PROV-O namespace, and process mining configuration when the kernel document URI matches.
 */
export interface TargetWorkflow {
  /**
   * Canonical URL of the target WOS Kernel Document. The ontology processor resolves the JSON-LD context, SHACL shapes, PROV-O namespace, and process mining configuration from this sidecar when the kernel document URI matches this value exactly.
   */
  url: string;
  /**
...
export interface ContextConfiguration {
  ...
  contextUrl: string;
  /**
   * Version of the context document. Synchronized with the WOS specification version. An ontology processor MUST reject a context document whose major version it does not support.
   */
