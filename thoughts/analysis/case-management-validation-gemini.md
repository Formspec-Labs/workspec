...
**Symptom:** The current system uses `WorkflowProcess` as the root product abstraction, leading to bloated `caseState` variables and a conflation of real-world matters with running workflow executions.
...
**Root Domino:** The architectural decision to make the WOS workflow instance (`WorkflowProcess`) the top-level entity, meaning any data collection (Formspec responses) currently has to live inside or be strictly bound to a workflow lifecycle state.
...
**Trace:** WOS runtime evaluator → `lifecycleHook` → transition tags. If `WorkflowProcess` = `Case`, completing the process closes the real-world matter.
...
- *Action Item:* Add to the implementation plan that the L0 Kernel `WorkflowProcess` (to become `CaseProcess`) MUST include `caseId` as a mandatory linkage field, and sidecars (e.g. `BusinessCalendar`) must retain their URL bindings to the `WorkflowDocument`.
...
- **Inconsistencies Surfaced:** The spec currently uses `$wosWorkflowProcess` as an internal marker. The document wisely recommends keeping this marker temporarily for backward compatibility before a hard rename to `$wosProcessInstance` in the canonical schemas (`instance.schema.json`).
