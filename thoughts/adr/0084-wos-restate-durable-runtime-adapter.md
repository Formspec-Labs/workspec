...
- **WS-094** in [`wos-spec/crates/wos-server/TODO.md`](../crates/wos-server/TODO.md) tracks the `wos-server-runtime-restate` crate: today it implements [`RuntimeOps`](../../crates/wos-server-ports/src/runtime.rs) over an in-memory `HashMap` with explicit unsupported errors for several operations — **not** yet wired to the Restate Server journal.
