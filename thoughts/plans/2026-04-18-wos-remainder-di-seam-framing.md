...
**Placement correction:** the explanation algorithm itself (§9.1) is runtime semantics — canonical home is **wos-runtime**, exposed as `wos_runtime::explain(&WorkflowProcess, &ProvenanceLog) -> ExplanationDoc`. The scaffold service inside server shrinks to an HTTP wrapper.
...
- **Path 2 — extend `ContractValidator` with `validate_in_context` default method.** Non-breaking addition: new trait method `validate_in_context(contract_ref, data, impact_level, process_id)` with a default impl delegating to `validate`. `PolicyLayeredValidator` overrides the default and performs §15.7 gating. Existing `ContractValidator` impls keep compiling. Simpler; preferred unless a stronger case surfaces for a separate trait.
...
Extend `WorkflowProcess.status` with first-class `declined | voided | expired` discriminators, each carrying required metadata:
...
- Schema: `schemas/companions/wos-process.schema.json` discriminated-union expansion.
...
- Each event type gets a canonical `data` shape (reference to the `WorkflowProcess`, `respondentLedgerRef` where applicable, ISO 8601 timestamp).
...
- **New trait** `IdentityResolver` in `crates/wos-core/src/traits/mod.rs`:
  ...
  pub trait IdentityResolver {
      ...
      fn resolve(
          ...
          instance_ctx: Option<&WorkflowProcess>,    // for delegation-chain resolution
...
- **New trait** `VisibilityService` in `crates/wos-core/src/traits/mod.rs`:
  ...
  pub trait VisibilityService {
      type Error: std::error::Error;
      fn case_status(&self, process_id: &str) -> Result<CaseStatusView, Self::Error>;
      fn case_file(&self, process_id: &str) -> Result<serde_json::Value, Self::Error>;
      fn governance_state(&self, process_id: &str) -> Result<GovernanceStateView, Self::Error>;
...
- **New trait** `TimerService` in `crates/wos-core/src/traits/mod.rs`:
  ...
  pub trait TimerService {
      ...
      fn cancel(&mut self, process_id: &str, timer_id: &str) -> Result<(), Self::Error>;
      fn list_pending(&self, process_id: &str) -> Result<Vec<TimerRef>, Self::Error>;
  }
...
- **New type** `TimerOp` in `crates/wos-core/src/timer.rs`:
  ...
  ```
- **Narrow `WorkflowProcess.timers`** from the current full-state `Vec<Timer>` to `Vec<TimerRef>` (id + fires_event + deadline snapshot, no duration origin). The origin-timestamp and duration-ISO move into the `TimerService` impl's storage.
...
**Migration cost.** Every fixture / test that asserts on `WorkflowProcess.timers[i].created_at_ms` or `duration_ms` needs updating — the origin info moves out. Grep `created_at_ms` gives the blast radius across runtime tests + fixture files. Scripted migration + manual review, one sprint.

**Why not skip.** The plan currently says "§7 Interop + speculative research — engine adapters held until a first-commercial-deployment-demands trigger fires." That's a reasonable posture for *building* the adapter, but the timer-state structural choice is a *shape-level* foreclosure that compounds with every new consumer of the current `WorkflowProcess.timers` shape. Land the shape fix before the blast radius grows; defer the adapter itself until demand fires.
...
Adapter-readiness fix. `InstanceStore::load(process_id)` returns a full snapshot; `InstanceStore::save(instance)` persists a full snapshot. Works fine for SQLite. Doesn't work for Temporal's event-sourced model — Temporal stores the event history, rehydrates state by replay; there's no "snapshot to save" because every activity result is already in the history.

Make this additive, not breaking:

- **New trait method** `InstanceStore::replay_events(process_id, from_seq) -> Result<Vec<StoredEvent>, Self::Error>` with a default impl returning `Ok(vec![])` (snapshot-only stores return nothing; callers that want event-sourcing check non-empty and replay).
- **New trait method** `InstanceStore::append_event(process_id, event) -> Result<EventSeq, Self::Error>` with default impl returning `Err(NotSupported)` — snapshot stores keep saving whole instances; event-sourced stores implement this instead of `save`.
- **Companion flag** on `InstanceStore`: `fn prefers_event_sourcing(&self) -> bool { false }` — the runtime uses this to decide whether to call `save(instance)` or `append_event(process_id, event)` after each transition.
...
- **H1 Timer extraction** — ~3 days, structural, D=5. The one item that must land even if the Temporal adapter never ships, because the `WorkflowProcess.timers` shape is compounding debt per fixture.
...
Track H. Timer extraction is the one high-blast-radius item — it touches `WorkflowProcess`, the evaluator's return type, every timer fixture, every timer test. Do it in one focused sprint rather than amortising.
...
Phase 7 (timer extraction) is best done by one engineer serially — parallel work on `WorkflowProcess.timers` creates merge-conflict hell. Schedule it at a natural lull in the spec work.
...
- `schemas/companions/wos-process.schema.json` — #58 envelope-status discriminator.
...
- `wos-core::instance::WorkflowProcess.timers` narrows from `Vec<Timer>` to `Vec<TimerRef>` (H1).
...
- `wos-core::traits::ContractValidator` — optionally extended with `validate_in_context(contract_ref, data, impact_level, process_id)` default method (B1 alternative path) if the submit-path `SubmitPolicy` approach isn't adopted. Non-breaking either way.
