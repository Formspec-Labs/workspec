...
A browser-based case management portal for WOS (Workflow Orchestration Standard). Provides an inbox, form workspace, case viewer, process dashboard, workflow designer, admin console, audit trail, applicant portal, and report builder — all backed by live kernel state synchronized over WebSockets.

> **Renamed 2026-05-02:** previously published as `@formspec-org/wos-studio` and located at `/studio` in this repo. Renamed to `@formspec-org/wos-case-portal` at `/case-portal` to free the `/studio` path for the WOS Studio (Authoring) layer (see `studio/README.md`). Same package, descriptive name.

The portal is a **reference implementation** that consumes `wos-spec` JSON Schemas and fixtures. It is not a Formspec renderer — the `@formspec-org/` npm scope is shared with the Formspec ecosystem, but this package has no runtime dependency on any `formspec-*` package. The `contractRef` / `binding` markers on tasks are surfaced as UI metadata; actual Formspec-backed forms are out of scope for this project at this time.
